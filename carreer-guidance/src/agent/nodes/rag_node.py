...
def run_rag(state: AgentRuntimeState, question: str | None = None) -> dict[str, Any]:
    ...
    resolved = str(getattr(state, "resolved_question", "") or "").strip()
    q = (question or resolved or state.question).strip()
    if not q:
        raise ValueError("question is required for RAG tool")

    if state.rag_tool_client is None:
        state.rag_tool_client = RAGTool(
            execution_id=state.execution_id, user_id=state.user_id
        )
