...
API_LLM_MODELS = {
    ...
    "groq-llama3-8b": {"provider": "groq", "name": "llama-3.1-8b-instant", "api_key_env": "GROQ_API_KEY", "max_tokens": 1024, "temperature": 0.3},
    "gemini-flash": {"provider": "gemini", "name": "gemini-2.0-flash", "api_key_env": "GEMINI_API_KEY", "max_tokens": 1024, "temperature": 0.3},
}
...
class LLMService:
    ...
    def __init__(self, *, execution_id: str, model_key: str | None = None):
        ...
        if not self.api_key:
            self.logger.warning("%s not set! Set it in .env or environment.", self.model_config["api_key_env"])
    ...
    def generate(self, question: str, context: str, language: str = "en", max_tokens: Optional[int] = None, temperature: Optional[float] = None) -> Dict:
        if not self._loaded:
            self.load()
        ...
        for attempt in range(max_retries):
            try:
                if self.provider == "groq":
                    response = self.client.chat.completions.create(model=self.model_name, messages=[{"role": "user", "content": prompt}], max_tokens=max_tok, temperature=temp)
                    answer = response.choices[0].message.content
                    tokens_gen = response.usage.completion_tokens if response.usage else len(answer.split())
                elif self.provider == "gemini":
                    response = self.client.generate_content(prompt, generation_config={"max_output_tokens": max_tok, "temperature": temp})
                    answer = response.text
                    tokens_gen = len(answer.split())
                break
            except Exception as e:
                self.logger.warning("API call failed (%d/%d): %s", attempt + 1, max_retries, e)
        ...
        return {"answer": answer.strip(), "latency_ms": latency, "tokens_generated": tokens_gen, "tokens_per_second": tokens_gen / (latency / 1000) if latency > 0 else 0, "provider": self.provider, "model": self.model_name}
    ...
    def translate_query(self, query: str, language: str = "en") -> str:
        ...
        try:
            if self.provider == "groq":
                response = self.client.chat.completions.create(model=self.model_name, messages=[{"role": "user", "content": prompt}], max_tokens=60, temperature=0.1)
                eng_query = response.choices[0].message.content
            elif self.provider == "gemini":
                response = self.client.generate_content(prompt, generation_config={"max_output_tokens": 60, "temperature": 0.1})
                eng_query = response.text
            else:
                return query
            eng_query = eng_query.strip(" '\"\n`")
            elapsed = (time.perf_counter() - start) * 1000
            self.logger.info("Translated VI→EN: '%s' → '%s' (%.0fms)", query, eng_query, elapsed)
            return eng_query
        except Exception as e:
